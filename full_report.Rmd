...
Before building the model, we first confirm the technical condition of proportional hazards is met by finding no significant p-values in the hypothesis test run below.

```{r}
cox.zph(coxph(Surv(time,censor) ~ tx + sex + raceth + ivdrug + hemophil + karnof + cd4 + age, data=data))
```

Then, we build the model below.
