...
ggarrange(p1, p2, p3, p4, p5, 
  ...
  annotate_figure(top = text_grob("Figure 9. Bootstrap Sampling Distribution Beta Coefficients"),
                  bottom = text_grob("Solid black line is the original beta. Dotted lines are +/-2*SE of original betas. Red lines are +/-2*SE of bootstrapped betas.", size = 8))
```
...
Now that we have validated our final model using Boostrapping, we interpret its output. The final model obtained after selecting features via LASSO was $$ln(\frac{h_i(t)}{h_0(t)}) = \beta_1tx1 + \gamma_1karnof80 + \gamma_2karnof90 + \gamma_3karnof100 + \theta_1 cd4$$. To better understand the effects of each predictor, we provide the following graphics. 
...
options(datadist = 'ddist')
options(digit = 3)
plot(summary(final.fit), log = TRUE, main = "Figure 10. The Estimated Hazard Ratios for Each Predictor's Values")
```
