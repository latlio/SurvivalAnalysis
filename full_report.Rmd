...
The goal of any GBM (gradient boosting machine) is to learn a functional mapping from the data $\{x_i, y_i\}_{i=1}^{n}$ to $y = F(x, \beta)$ where $\beta$ is the set of parameters of the function $F$ which minimize some cost function $\sum_{i=i}^{n} \Phi(y_i, F(x_i; \beta))$. An important assumption of boosting is that $F(x) = \sum_{m=0}^{M} \rho_m f(x; \tau_m)$ where $f$ is a "weak" learner with a weight $\rho$ and set parameter $\tau_m$ (described just below). In other words, this assumption means that $F(x)$ follows an "additive" expansion form, permitting us to perform "additive training" or boosting. Otherwise, there are no functional assumptions made on $F$, allowing it to be linear, tree-based, or otherwise. Thus, the set of parameters of the function $F$ is exactly $\beta = \{\rho_m, \tau_m\}_{m=1}^{M}.$ These are learned in a greedy iterative process, detailed below:

\begin{enumerate}
  \item Set an initial estimator $f_0(x)$
  \item For each iteration $m \in \{1,\dots,M\}$, solve:
  $$(\rho_m, \tau_m) = \arg\min_{\rho,\tau} \sum_{i=1}^n\Phi(y_i;F_{m-1}(x_i) + \rho f(x_i; \tau))$$
  We next approximate this second step by
    \begin{enumerate}
      \item Fit $f(x; \tau_m)$ by $$\tau_m = \arg \min_{\tau} \sum_{i=1}^n (g_{im} -f(x_i;\tau))^2$$ where $$g_{im} = -[\frac{\delta \Phi(y_i, F(x_i))}{\delta F(x_i)}]_{F(x) = F_{m-1}(x)}$$
      \item Learn $\rho$ by $$\rho_m = \arg \min_{\rho}  \sum_{i=1}^n \Phi(y_i, F_{m-1}(x_i) + \rho f(x_i;\tau_m)) $$
    \end{enumerate}
    \item Update $F_m(x) = F_{m-1}(x) + \nu\rho_m f(x; \tau_m)$ for some learning rate or shrinkage term, $\nu \in (0,1]$ (this is called `eta` in the XGBoost code). The shrinkage term is used to control overfitting and typically is valued between $0.01-0.2$. The complexity of $f(x)$ is informed by the tree depth and minimum number of samples in terminal nodes. We can also improve GBM performance by subsampling, or fitting each weak learner on a random subset of the training data, known as stochastic gradient boosting.  
\end{enumerate}

To boost a proportional hazards model, the cost function becomes the negative partial log-likelihood:
...
## Analysis
See the XGBoost jupyter notebook for the analysis (I was really struggling in getting Python to work in RStudio, so putting mycode in this markdown feels like more trouble than it is worth right now!).
