...
```{r}
ggplot(data %>% filter(censor == 0) %>% mutate(tx=ifelse(tx == 0, "Control", "IDV")), aes(x = time_d)) + geom_histogram(bins = 20) + facet_grid(.~tx) + ggtitle("Incomplete Observations: Treatment vs. Time to Event") + xlab("Time until End of Study")
```
