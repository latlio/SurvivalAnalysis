...
```{r}
f_aic <- selectCox(Surv(time,censor)~tx+sex+raceth+ivdrug+hemophil+karnof+cd4+priorzdv+age, data=data, rule="aic")
f_aic
cox.zph(coxph(Surv(time,censor) ~ karnof+cd4, data=data))
```
...
```{r}
f_p <- selectCox(Surv(time,censor)~tx+sex+raceth+ivdrug+hemophil+karnof+cd4+priorzdv+age, data=data, rule="p")
f_p
cox.zph(coxph(Surv(time,censor) ~ karnof+cd4, data=data))
```
...
```{r}
cox.zph(coxph(Surv(time,censor) ~ (tx+sex+raceth+ivdrug+hemophil+karnof+cd4+priorzdv+age)^2, data=data))
```

```{r}
f_aic <- selectCox(Surv(time,censor)~(karnof+cd4)^2, data=data, rule="aic")
f_aic
cox.zph(coxph(Surv(time,censor) ~ (karnof+cd4)^2, data=data))
```


# Somthing New: Power Analysis Simulation

Madison will be performing power analysis using simulation to determine the optimal sample size. 
